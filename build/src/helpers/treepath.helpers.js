...
const stream_worker_1 = __importDefault(require("stream-worker"));
    ...
    listToTree(list, sort) {
        ...
        for (index = 0; index < list.length; index += 1) {
            ...
            if (hasParentInMap) {
                ...
                if (shouldSort) {
                    list[nodeMap[currentNode.parent]].children = orderBy_1.default(list[nodeMap[currentNode.parent]].children, lodashSort.keys, lodashSort.orders);
                }
            }
            else {
        ...
        if (shouldSort) {
            rootNodes = orderBy_1.default(rootNodes, lodashSort.keys, lodashSort.orders);
        }
        return rootNodes;
    }
}
exports.default = (schema, options) => {
    const onDelete = (options && options.onDelete) || "REPARENT";
    const idType = (options && options.idType) || mongoose_1.Schema.Types.ObjectId;
    ...
    schema.pre("save", function preSave(next) {
        ...
        const updateChildPaths = (pathToReplace, replacementPath) => {
            ...
            stream_worker_1.default(childStream, onStreamData, streamWorkerOptions, onStreamClose);
        };
    ...
    schema.statics.getChildrenTree = function getChildrenTree(args) {
        ...
        return this.find(filters, fields, options)
            ...
            .then((result) => result.filter((node) => {
